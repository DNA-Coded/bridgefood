...
export const Home: React.FC<HomeProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-12 pb-8 overflow-x-hidden">
      ...
      <section className="relative overflow-hidden rounded-xl bg-cover bg-center min-h-[480px] flex items-center justify-center p-8 md:p-16 border border-slate-200 dark:border-slate-800" style={{ backgroundImage: "url('/hero-bg.webp')" }}>
        <div className="absolute inset-0 bg-[#171d16]/55 backdrop-blur-[0.5px]"></div>
        <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center justify-center text-center space-y-7">
          <span className="inline-flex items-center rounded-sm border border-primary/30 bg-[#171d16]/80 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
          ...
          <div className="space-y-4">
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight text-white md:text-6xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
              ...
              into <span className="text-[#62df7d]">community impact</span>.
            </h1>
            <p className="max-w-2xl text-base leading-7 text-slate-200 md:text-lg drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] font-medium">
          ...
          <div className="flex flex-col gap-3 sm:flex-row justify-center w-full max-w-md">
            <Button onClick={() => onNavigate('/role-selection')} size="lg" className="w-full sm:w-auto bg-[#006b2c] text-white hover:bg-[#005320] font-bold rounded-sm">
              Donate Food <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
            <Button onClick={() => onNavigate('/organizations/discover')} variant="outline" size="lg" className="w-full sm:w-auto border-white/40 bg-white/10 text-white hover:bg-white/20 hover:text-white font-bold rounded-sm">
              Find NGOs
            </Button>
          </div>
          <div className="flex flex-wrap justify-center gap-4 text-xs font-semibold text-slate-300 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-[#62df7d]" /> Verified partners</span>
            <span className="inline-flex items-center gap-1.5"><Truck className="h-4 w-4 text-[#62df7d]" /> Dispatch-ready pickups</span>
            <span className="inline-flex items-center gap-1.5"><HeartHandshake className="h-4 w-4 text-[#62df7d]" /> Impact records</span>
          </div>
        </div>
      </section>
